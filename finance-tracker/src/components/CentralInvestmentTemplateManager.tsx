...
interface CentralInvestmentTemplateManagerProps {
  centralInvestmentTemplate: CentralInvestmentTemplate | null;
  onUpdateTemplate: (template: CentralInvestmentTemplate) => void;
  onCreateTemplate: () => void;
}

export default function CentralInvestmentTemplateManager({
  centralInvestmentTemplate,
  onUpdateTemplate,
  onCreateTemplate,
}: CentralInvestmentTemplateManagerProps) {
  const [showForm, setShowForm] = useState(false);
  const [editingItem, setEditingItem] = useState<InvestmentTemplateItem | null>(null);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!centralInvestmentTemplate) return;

    const itemData: InvestmentTemplateItem = {
    ...
    if (editingItem) {
      updatedItems = centralInvestmentTemplate.items.map(item =>
        item.id === editingItem.id ? itemData : item
      );
    } else {
      updatedItems = [...centralInvestmentTemplate.items, itemData];
    }

    const updatedTemplate: CentralInvestmentTemplate = {
      ...centralInvestmentTemplate,
      items: updatedItems,
      updatedAt: new Date(),
    };
  ...
  const handleEdit = (item: InvestmentTemplateItem) => {
    setEditingItem(item);
    setFormData({
      name: item.name,
  ...
  const handleDelete = (itemId: string) => {
    ...
    const updatedItems = centralInvestmentTemplate.items.filter(item => item.id !== itemId);
    const updatedTemplate: CentralInvestmentTemplate = {
      ...centralInvestmentTemplate,
      items: updatedItems,
      updatedAt: new Date(),
  ...
  const totalTemplateAmount = centralInvestmentTemplate?.items.reduce((sum, item) => sum + item.amount, 0) || 0;
    ...
    <div className="bg-gray-800 rounded-lg p-4 sm:p-6 mb-4 sm:mb-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg sm:text-xl font-semibold">Investment Template Manager</h2>
        </div>
      </div>
      ...
      {!centralInvestmentTemplate ? (
        <div className="text-center text-gray-400 py-8">
          ...
          <p className="mb-2 text-sm sm:text-base">No investment template found</p>
          <p className="text-xs sm:text-sm mb-4">Create an investment template to manage your recurring investments</p>
          <button
        ...
        <>
          ...
          <div className="bg-gray-700 rounded-lg p-3 sm:p-4 mb-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              <div>
                ...
                <p className="text-gray-400 text-xs">Total monthly recurring investments</p>
              </div>
              <div className="text-left sm:text-right">
                <p className="text-gray-400 text-xs">This template will be copied</p>
                <p className="text-gray-400 text-xs">when you fill monthly investments</p>
              </div>
            </div>
          </div>
          ...
          {/* Template Items List */}
          {centralInvestmentTemplate.items.length > 0 ? (
            <div className="space-y-2 sm:space-y-3">
              <h3 className="font-medium mb-2 sm:mb-3 text-sm sm:text-base">Investment Template Items</h3>
            ...
            <div className="text-center text-gray-400 py-6 sm:py-8">
              <p className="mb-2 text-sm sm:text-base">No investment template items yet</p>
              <p className="text-xs sm:text-sm">Add items like SIP, FD, stocks, etc.</p>
            </div>
          )}
        </>
      )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-lg p-6 w-full max-w-md">
            <h3 className="text-lg font-semibold mb-4">
              {editingItem ? 'Edit Investment Item' : 'Add Investment Item'}
            </h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Monthly SIP, Fixed Deposit, Gold ETF"
                  required
                />
              </div>
